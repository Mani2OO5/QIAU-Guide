...
def build_vector_database(data):
    ...
    chroma_client = chromadb.PersistentClient(path="uni_data/chroma_db")
        ...
        while True:
            try:
                response = client.models.embed_content(
                    model=EMBED_MODEL, contents=batch_documents
                )

                embeddings = [embedding.values for embedding in response.embeddings]

                collection.add(
                    ids=batch_ids, documents=batch_documents, embeddings=embeddings
                )
                ...
                if e.code == 429:
                    ...
                    for detail in e.details.get("error", {}).get("details", []):
                        if "retryDelay" in detail:
                            ...
                            break
                    print(f"waiting: {retry_delay} second")
...
def embedding_search(query, n_results=5):
    chroma_client = chromadb.PersistentClient(path="uni_data/chroma_db")
    collection = chroma_client.get_collection(name="telegram_messages")
...
if __name__ == "__main__":
    os.makedirs("uni_data", exist_ok=True)

    dotenv.load_dotenv()
    EMBED_MODEL = os.getenv("GOOGLE_EMBED_MODEL")
    GENAI_CLIENT = os.getenv("GEMINI_API_KEY")
    client = genai.Client(api_key=GENAI_CLIENT)
